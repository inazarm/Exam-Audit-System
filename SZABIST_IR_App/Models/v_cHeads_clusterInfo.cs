//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SZABIST_IR_App.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class v_cHeads_clusterInfo
    {
        public byte tCluster_Head_Id { get; set; }
        public Nullable<byte> tCluster_ID { get; set; }
        public Nullable<byte> tMemberID { get; set; }
        public string ClusterHeadID { get; set; }
        public string ClusterHeadName { get; set; }
        public Nullable<System.DateTime> WEF_From { get; set; }
        public Nullable<System.DateTime> WEF_To { get; set; }
        public Nullable<byte> CampusID { get; set; }
        public Nullable<bool> HeadStatus { get; set; }
        public string ClusterTitle { get; set; }
        public string ClusterDescription { get; set; }
        public Nullable<bool> ClusterStatus { get; set; }
    }
}
